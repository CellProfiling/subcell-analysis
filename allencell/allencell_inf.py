...
if __name__ == "__main__":
    ...
    args = argparser.parse_args(["-c", "configs/config_subcell_mae_allen.yaml"])
    ...
    for config in configs:
        ...
        inference_folder = f"{inference_folder}/{config['name']}_channel_comb_{config['channel_combination']}_norm_{config['normalization']}"
        os.makedirs(inference_folder, exist_ok=True)
        ...
        print(
            f"Starting inference for {config['name']}_channel_comb_{config['channel_combination']}_norm_{config['normalization']}"
        )

        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        dataset = AllenCellDataset(
            config["crops_folder"],
            ...
            for idx, (batch_images, batch_images_orig, batch_df) in tqdm(
                ...
                if idx % 25 == 0 and config.get("save_attention", False):
                    ...
                    save_attention_images(
                        ...
                        color_channels=["cyan", "pink", "white"],
                        normalization="allencell",
                        metadata=batch_df,

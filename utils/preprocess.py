...
def load_crop(img_path, bbox, crop_params):
    image = np.stack(
        [
            cv2.imread(f"{img_path}masked_{color}.png", -1)
            for color in crop_params["channels"]
        ],
        axis=-1,
    )
    pad = crop_params.get("pad")
    crop = crop_params.get("crop")
    resize_to = crop_params.get("resize_to")

    if pad > 0:
        bbox = bbox + np.array([-pad, -pad, pad, pad])
    if crop > 0:
